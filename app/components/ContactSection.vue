      ...
      <div class="grid lg:grid-cols-2 gap-10">
        <div class="space-y-5 reveal-left">
          <div class="card-surface rounded-2xl p-6 space-y-5">
            ...
            <div class="space-y-3">
              <a v-for="contact in contactItems" :key="contact.label" :href="contact.href"
                class="flex items-center gap-3 p-3 card-surface-light rounded-xl hover:border-violet-500/30 transition-all group">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                  :style="{ background: `${contact.color}15` }">
                  <UIcon :name="contact.icon" class="w-3.5 h-3.5" :style="{ color: contact.color }" />
                </div>
                <div class="min-w-0 flex-1">
                  <p class="text-stone-600 text-xs">{{ contact.label }}</p>
                  <p class="text-stone-200 text-xs sm:text-sm font-medium group-hover:text-violet-300 transition-colors truncate">
                    {{ contact.value }}
                  </p>
                </div>
                <UIcon name="i-heroicons-arrow-up-right" class="w-3 h-3 text-stone-700 flex-shrink-0 group-hover:text-violet-400 transition-colors" />
              </a>
            </div>
          ...
          <div class="card-surface rounded-2xl p-6">
            ...
            <form class="space-y-4" @submit.prevent="handleSubmit">
              ...
              <button type="submit" :disabled="sending || sent"
                class="btn-primary w-full py-3.5 rounded-xl font-semibold text-sm disabled:opacity-60 flex items-center justify-center gap-2">
                <template v-if="sent">
